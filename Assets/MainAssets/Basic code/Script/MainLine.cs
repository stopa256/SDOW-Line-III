...
public class MainLine : MonoBehaviour
{
   ...
   [HideInInspector] public Material LineMaterial,DiamondMaterial;
	...
	void Start ()
    {
        ...
        if (Skin.Head) Head = Skin.Head;
        if (Skin.Tail) Tail = Skin.Tail;
        if (Skin.TapEffect) TapEff = Skin.TapEffect;
        if (Skin.StaticEffect) StaEff = Skin.StaticEffect;
        SetSkin();
    ...
    public void SetSkin()
    {
        if (Head)
        {
            ...
            HeadEffIns =  Instantiate(Head, this.gameObject.transform);
            HeadEffIns.transform.localPosition = new Vector3(0, 0, 0) + Skin.HeadOffset;
        }
        if (Tail)
        {
            tail = Tail;
        ...
        if (StaEff)
        {
            Instantiate(StaEff, this.gameObject.transform);
        }
    }
    private void TapEffect()
    {
        if (TapEff)
        {
            Destroy(TapEffIns =  Instantiate(TapEff, this.gameObject.transform), 5f);
            TapEffIns.transform.localPosition = new Vector3(0, 0, 0) + Skin.TapEffectOffset;
        }
    }
        ...
        if (win != true)
			...
			if (Crown1 != null)
            {
				if (ContinueTab != null)
                {
					...
					DOTween.To (() => ContinueTab.GetComponent<CanvasGroup> ().alpha, x => ContinueTab.GetComponent<CanvasGroup> ().alpha = x, 1, 0);
				}
			}
            else
            {
				if (OverTab != null)
                {
					...
					DOTween.To (() => OverTab.GetComponent<CanvasGroup> ().alpha, x => OverTab.GetComponent<CanvasGroup> ().alpha = x, 1, 0);
    ...
    private GameObject StaEff;
    private GameObject TapEffIns;
    private GameObject HeadEffIns;
}